...
for i = [2 1] %length(conds):-1:1                                                  % over conditions
        ...
        try                                                                 % trying a subject
            ...
            for j = 1:length(subconds)                                      % over sub conds (tasks)
                try                                                         % trying a subcondition
                    ...
                    %% ICA
%                     EEG = fastica(EEG, 'extended',5,'interrupt','on');
                    EEG = pop_runica(EEG, 'extended',5);
                    [ALLEEG,EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);

                    %% preparing variables to save (includes all subconds)
                    % Components = Weights * sphere * Data
                    final.(subconds{j}).sph = EEG.icasphere;
                    final.(subconds{j}).w = EEG.icaweights;
